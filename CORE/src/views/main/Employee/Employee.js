...
import { Backdrop, CircularProgress, Input, LinearProgress, MenuItem, TextField } from '@mui/material'
...
const Employee = () => {
  ...
  var [employeeList, setEmployeeList] = useState(new Array())
  ...
  const genderList = [
    { label: t('gender'), value: null },
    { label: t('male'), value: 'male' },
    { label: t('female'), value: 'female' }
  ];

  const religionList = [
    { label: t('religion'), value: null },
  ...
  const getEmployee = (_page, _perPage, searchText) => {
    GetEmployees(_page, _perPage, searchText, companyId)
      .then((response) => {
  ...
  const getRoles = () => {
    axios({
      ...
      .then((response) => {
        ...
        roleData.map((item) => {
          __roleList.push({ label: item.name, value: item.id })
        })
        setRoleList(__roleList)
      })
      .catch((error) => {
        //
      })
  ...
  async function openForm() {
    if (rolePermissionEmployee.permissionCreate) {
      setPageType('new')
      defaultValue()
  ...
  const clickRow = async (row, event) => {
    if (rolePermissionEmployee.permissionEdit) {
      try {
        const employeeDetail = await GetById(row.id)
        let user = new User(employeeDetail.user)
        setAlertVisible(false)
        setPageType('edit')
  ...
  async function addData() {
    ...
    if (!(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(validationCheck.email))) {
      setAlertMsg(`Harap masukkan karyawan email valid `);
      return handleClick()
    }
          ...
          if (i === documents.length - 1) {
            ...
            axios({
              ...
              .then((response) => {
                ...
                setPageType('new')
                setVisible(false)
                getEmployee(page, perPage, '', companyId)
              })
              .catch((error) => {
                setLoading(false)
                setAlertMessage(
  ...
  function search(event) {
    getEmployee(page, perPage, event.toLowerCase(), companyId)
  }
  ...
  let findNavigate = findConfigPage.items.find((e) => e.name.toLowerCase() === 'employee')
        ...
        <CRow>
          ...
          >
            ...
            <CModalHeader closeButton={false}>
              <CModalTitle>{pageType === 'new' ? `${t('add')} ${t('employee')}` : fullName}</CModalTitle>
            </CModalHeader>
            <CModalBody>
