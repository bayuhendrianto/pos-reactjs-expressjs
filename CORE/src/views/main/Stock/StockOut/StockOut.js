...
const StockOut = () => {
    ...
    const rolePermissionStockOut = new PermissionModel(
        rolePermission.find((perm) => perm.permissionName?.toLowerCase().replace(/\s+/g, "") === 'stockout'),
    )
    let user = new User(useSelector((state) => state.user))
    const companyId = useSelector((state) => state.companyId)
    var [stockOutList, setStockOutList] = useState(new Array())
    ...
    useEffect(() => {
        setSearchText('')
        getStockOut(page, perPage, searchText);
        defaultValue();
        setLoading(false);
        // generateDate()
    }, [])
    ...
    const getStockOut = (_page, _perPage, searchText) => {
        axios({
            url: `/stock-out?page=${_page}&size=${_perPage}&search=${searchText}`
        }).then((response) => {
            setStockOutList(response.data.result)
    ...
    const handlePageChange = (page) => {
        ...
        setPage(_page)
        getStockOut(_page, perPage, searchText)
    }

    const handlePerRowsChange = async (usePerPage, _page) => {
        setPerPage(usePerPage)
        getStockOut(page, usePerPage, searchText)
    }

    async function openForm() {
        if (rolePermissionStockOut.permissionCreate) {
            naviage('/stock-out/new')
        } else {
    ...
    const clickRow = async (row, event) => {
        if (rolePermissionStockOut.permissionEdit) {
            naviage(`/stock-in/${row.id}`)
        } else {
            setAlertRoleVisible(true)
            setAlerRole(`Sorry... You don't have access !`)
        }
    ...
    function search(value) {
        setSearchText(value);
        getStockOut(page, perPage, value.toLowerCase())
    }
