...
const Supplier = () => {
    ...
    const rolePermissionSupplier = new PermissionModel(
        rolePermission.find((perm) => perm.permissionName === 'Supplier'),
    )
    ...
    var [supplierList, setSupplierList] = useState(new Array())
    ...
    const getSuppliers = (_page, _perPage, searchText) => {
        axios({
            url: `/supplier?page=${_page}&size=${_perPage}&search=${searchText}`,
            method: 'get',
            data: {},
        })
            .then((response) => {
    ...
    const handlePageChange = (page) => {
        ...
        setPage(_page)
        getSuppliers(_page, perPage, '')
    }

    const handlePerRowsChange = async (userPerPage, _page) => {
        setPerPage(userPerPage)
        getSuppliers(page, userPerPage, '')
    }

    async function openForm() {
        if (rolePermissionSupplier.permissionCreate) {
            setPageType('new')
            defaultValue()
    ...
    const clickRow = async (row, event) => {
        if (rolePermissionSupplier.permissionEdit) {
            try {
                const supplierDetail = await GetById(row.id)
                let user = new User(supplierDetail)
                setAlertVisible(false)
                setPageType('edit')
    ...
    function search(event) {
        getSuppliers(page, perPage, event.toLowerCase())
    }
    ...
    let findNavigate = findConfigPage.items.find((e) => e.name.toLowerCase() === 'supplier')
                ...
                <CRow>
                    ...
                    >
                        ...
                        <CModalHeader closeButton={false}>
                            <CModalTitle>{pageType === 'new' ? `${t('add')} ${t('supplier')}` : fullName}</CModalTitle>
                        </CModalHeader>
                        <CModalBody>
