...
const Category = () => {
    ...
    const getCategories = (_page, _perPage, searchText) => {
        GetCategories(_page, _perPage, searchText)
            .then((response) => {
    ...
    const handlePageChange = (page) => {
        ...
        setPage(_page)
        getCategories(_page, perPage, '')
    }

    const handlePerRowsChange = async (userPerPage, _page) => {
        setPerPage(userPerPage)
        getCategories(page, userPerPage, '')
    }

    function GetCategories(page, size, searchText) {
        return new Promise((resolve, reject) => {
            axios({
                url: `/category?page=${page}&size=${size}&search=${searchText}`,
                method: 'get',
            })
                .then((response) => {
    ...
    function openForm() {
        let permissionRole = rolePermission.find(
            (perm) => perm.permissionName.toLowerCase() === 'category',
        )
        if (permissionRole) {
    ...
    function submitForm() {
        ...
        axios({
            url: pageType === 'new' ? '/category/new' : '/category/update',
    ...
    function search(event) {
        getCategories(page, perPage, event.toLowerCase())
    }
    ...
    let findPage = findConfigPage.items.find((e) => e.name.toLowerCase().replace(/\s+/g, "") === 'category')
                ...
                <CRow>
                    ...
                    >
                        ...
                        <CModalHeader closeButton={false}>
                            <CModalTitle>{pageType === 'new' ? 'tambah kategori' : name}</CModalTitle>
                        </CModalHeader>
                        <CModalBody>
                            <CForm>
                                <CRow>
