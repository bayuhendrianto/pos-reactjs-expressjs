...
const WidgetsDropdown = () => {
  ...
  const [countBookingStatus, setCountBookingStatus] = useState(0)
  ...
  useEffect(() => {
    GetCountBooking(), GetCountBookingStatus('NEW')
   ...
   * @param {*} status
   */
  function GetCountBookingStatus(status) {
    axios({
      url: `/booking/count-status/${status}`,
      method: 'get',
    })
      .then((response) => {
        setCountBookingStatus(response.data.count)
      })
      .catch((error) => {
        console.log(error)
      })
